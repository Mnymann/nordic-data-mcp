...
### Added
- **Real prompts, resources, and a server instruction (Smithery quality).** The empty `resources`/`prompts` stubs from 1.5.1 silenced the `-32601` warnings but earned no quality points — Smithery scores *real* discovery content. This release adds:
  - **3 documentation resources** — `nordic://countries` (country coverage + tier-gating + VAT codes), `nordic://tools` (catalog of all 11 tools), and `nordic://getting-started`. Served over `resources/list` + `resources/read` (plus an empty `resources/templates/list`).
  - **3 workflow prompts** — `due_diligence`, `vat_check`, and `sanctions_screening`. Served over `prompts/list` + `prompts/get`.
  - **Server `instructions`** ("system prompt") returned in `initialize`, telling agents how to pick a tool and avoid the UK-vs-GB / country-casing pitfalls.
  - All new content is static (no upstream calls, no PII), so `resources/read`, `resources/templates/list`, and `prompts/get` are added to the keyless discovery allow-list. No tool behaviour changes — still 11 tools.
