...
| `mcp-publisher login github` → `dial tcp: lookup github.com: no such host` (but github.com opens fine in the browser) | macOS system DNS resolver has a stuck negative cache for github.com; browsers bypass it via their own DoH so they still work. `/etc/hosts` is clean and `npm publish` succeeded moments earlier, which rules out a real outage. | Flush DNS: `sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder` (one Mac password prompt, silent on success), then retry the login. |
