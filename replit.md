...
**Current shipped version: 1.5.1** (May 31, 2026). Live on NPM (`nordic-data-mcp@1.5.1`), Railway (`/healthz` reports 1.5.1), and the official MCP Registry (`io.github.Mnymann/nordic-data@1.5.1`). The `mcp-publisher` CLI binary lives at `/Users/martinnymann/mcp-registry/bin/mcp-publisher` on Martin's Mac.

Release history this weekend:
- **1.5.3** (May 31) — **Remote-connect clarity + public-endpoint guardrail (backlog #7, model A).** Diagnosed live: a generic, unconfigured MCP client can't connect to `/mcp/auth` because on a 401 it falls back to OAuth DCR (`POST /register` → 404). Root cause is client-side default behavior — the server advertises NO OAuth (no `WWW-Authenticate`, no well-known metadata; verified live). Decision (Martin): **model A — keep API-key auth, defer OAuth to Phase 2.** Changes: (1) `/register` + OAuth `/.well-known/*` now return a clear JSON **4xx** `oauth_not_supported` (never 200, which would make an OAuth client think it registered); (2) added a per-IP rate limit on the public `/mcp` (defense-in-depth — the keyless `/mcp` bills upstream to the SERVER key, so it's a cost vector; backend per-key quota is the hard ceiling) with `trust proxy=1`; (3) README "Connecting a remote client" guide (header clients → `/mcp/auth` Bearer; generic clients → public `/mcp`; local → npx stdio). No tool/transport behavior change. **Ships together with 1.5.2 in one push** (prod is still 1.5.1). NOTE for #1: the "bound quota on the server key" lives on the backend — confirm the server key's tier/quota in the AddonNordic dashboard; the MCP-layer rate limit is additive, not a replacement.
- **1.5.2** (May 31) — **Smithery quality: real discovery surface.** Built & verified locally (typecheck/build/HTTP-smoke/security-check/discovery-probe all green, architect review passed); **pending Martin's push to Railway** (all the Smithery score needs) + optional Mac NPM+registry republish for distribution consistency. Replaced the empty 1.5.1 resources/prompts stubs with REAL content in BOTH transports: 3 resources (`nordic://countries`, `nordic://tools`, `nordic://getting-started`), 3 prompts (`due_diligence`, `vat_check`, `sanctions_screening`), and a server `instructions` system-prompt (returned in `initialize`). Added `resources/read`, `resources/templates/list`, `prompts/get` to `UNAUTH_METHODS` (static content, no upstream calls, safe keyless). **Lesson:** the 1.5.1 empty handlers AND a repo-root `icon.svg` both left the score stuck at 79 — for a remote/self-hosted server Smithery scores *real* prompts/resources + `instructions`, not empty capabilities or repo files. Still 11 tools.
- **1.5.1** (May 31) — **Registry quality fix.** Still a tools-only server (11 tools, no behavior change), but now declares the `resources` + `prompts` capabilities and answers `resources/list` / `prompts/list` with empty arrays. Previously these returned `-32601 Method not found`, which Smithery's inspector flagged and which lowered its Quality Score below 80. Handlers added in both transports (`src/index.ts` stdio + `src/http.ts` `buildServer()`); `UNAUTH_METHODS` already allow-listed both methods so the keyless-discovery boundary stays consistent.
- **1.5.0** (May 31) — **Hybrid discovery.** Added 3 runtime meta-tools (`list_endpoints`, `get_endpoint_schema`, `call_endpoint`) that read the backend's live OpenAPI spec, so agents reach all ~233 data endpoints without a new release. The 8 curated tools are unchanged → **11 tools total**. Security-hardened: `/admin/*` filtered from discovery AND hard-blocked in `call_endpoint` (path canonicalized — case-fold + iterative percent-decode + dot-segment; final substituted path re-validated post-`encodeURIComponent` to kill double-encoding smuggling), HTTP-method whitelist per spec, and API-key redaction on every egress point. `/api/dashboard/*` is hidden from discovery (operational, not data) but still callable. Added `npm run security-check` regression matrix. Spec is cached 5 min with stale-fallback.
...
- **RESOLVED — remote-MCP auth model = API-key header (model A); OAuth deferred to Phase 2.** Generic clients given only a URL try OAuth DCR (`POST /register`) and fail because this server uses static API-key auth and advertises no OAuth (no `WWW-Authenticate`, no well-known metadata — verified live against prod). Decision: keep API-key auth. Header-capable clients use `/mcp/auth` (Bearer); generic/auto-discovery clients use the public `/mcp` (no key, freemium). `/register` + OAuth `/.well-known/*` now return a clear JSON 4xx `oauth_not_supported` (never 200). Public `/mcp` got a per-IP rate limit (defense-in-depth on top of the backend per-key quota). **Full OAuth 2.1 (Phase 2) only when a real external self-onboarding customer appears** — building it speculatively is wasted timing.
- **RESOLVED — `list_endpoints` accounting is canonical, stop re-litigating it.** Live spec = **246 paths**. Discovery filters out **13**: 7 `/admin/*` routes + `/openapi.json` (the spec itself) + 5 `/api/dashboard/*` paths (6 endpoints, since `/api/dashboard/config` has GET+PUT). That leaves **233 unique data paths** = **233 endpoints** (the only multi-method path, `dashboard/config`, is now filtered, so paths and endpoints match 1:1). No legitimate data route is lost. Admin match is anchored (`=== "/admin"` or `startsWith("/admin/")` after canonicalization), so paths merely *containing* "admin" (`/administrator`, `/api/admin-notes/...`) are NOT blocked; dashboard match is anchored the same way (`/api/dashboard` exact or `/api/dashboard/` prefix).
- **RESOLVED — `/api/dashboard/*` filtered out of discovery.** The 6 dashboard routes (operational config/usage/stats/health, behind `INTERNAL_API_KEY` — the scoped MCP key gets 401, so no security risk) no longer appear in `list_endpoints`. Implemented as a discovery-only filter (`isDashboardPath` in `specClient.ts`, applied in `listDataEndpoints` alongside the `/openapi.json` exclusion). NOT a hard block: `call_endpoint` still permits dashboard paths (they just 401 for the scoped key) — this is surface cleanliness, not a security boundary like admin.
